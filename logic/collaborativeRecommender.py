...
class CollaborativeRecommender(Recommender):
    ...
    def cleanTheData(self, objectsToReturn, Rating_avg):
        check = pd.pivot_table(Rating_avg,values='rating_x',index='userId',columns='songId')
        #print(check.head())
        final = pd.pivot_table(Rating_avg,values='adg_rating',index='userId',columns='songId')
        #print(final.head())
    ...
    def find30neighboursForEachUser(self, objectsToReturn, similarity_with_user, similarity_with_song):
        sim_user_30_user = self.findNneighbours(similarity_with_user,30)
        #print(sim_user_30_user.head())

        sim_user_30_song = self.findNneighbours(similarity_with_song,30)
       # print(sim_user_30_song.head())

        objectsToReturn.append(sim_user_30_user)
        objectsToReturn.append(sim_user_30_song)
    ...
    def getUserSimilarSongs(self, user1, user2, Rating_avg):
        ...
        on = "songId",
        how = "inner" )
        return common_songs.merge( self.songs, on = 'songId' )

    def userItemScore(self, user, item,
                        sim_user_30_song, final_song,
                        Mean, similarity_with_song):
        a = sim_user_30_song[sim_user_30_song.index==user].values
        b = a.squeeze().tolist()
    ...
    def userItemRecommendedSongs(self, user, check, sim_user_30_song, Song_user, 
        ...
        data = pd.DataFrame({'songId':Songs_under_consideration,'score':score})
        top_5_recommendation = data.sort_values(by='score',ascending=False).head(5)
        Song_Name = top_5_recommendation.merge(self.songs, how='inner', on='songId')
        Song_Names = Song_Name.title.values.tolist()
        return Song_Names


    def getRecommendedSongs(self, Rating_avg, check, sim_user_30_song,
                           final_song, Mean, similarity_with_song):
        Rating_avg = Rating_avg.astype({"songId": str})
        Song_user = Rating_avg.groupby(by = 'userId')['songId'].apply(lambda x:','.join(x))
        user = 370 #the user for which we're recommending
        predicted_songs = self.userItemRecommendedSongs(user, check, sim_user_30_song,
    ...
    def recommend(self):
        ...
        objectsToReturn = []
        self.find30neighboursForEachUser(objectsToReturn, similarity_with_user, similarity_with_song)
        sim_user_30_user = copy.deepcopy(objectsToReturn[0])
        sim_user_30_song = copy.deepcopy(objectsToReturn[1])

       # a = self.getUserSimilarSongs(370,86309, Rating_avg)
       # a = a.loc[ : , ['rating_x_x','rating_x_y','title']]
        #print(a.head())

        #the user for which we're recommending = 370
        partialScore = self.userItemScore(370, 7371, 
                                          sim_user_30_song, 
                                          final_song, 
                                          Mean,
        ...
        self.getRecommendedSongs(Rating_avg, check, sim_user_30_song, final_song, Mean, similarity_with_song)
